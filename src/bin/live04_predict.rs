...
const STATE_OUTPUT: &str = "data/live04-state.json";
const SHARE_LOG: &str = "data/live04-shares.csv";
...
const CANDIDATE_POOL: usize = 8192;
const COLLAPSE_K: usize = 1024;
...
fn load_runtime() -> Runtime {
    ...
    r.predicted_lz8 = u("predicted_lz8");
    r.predicted_lz12 = u("predicted_lz12");
    r.predicted_lz16 = u("predicted_lz16");
    r.random_hashes = u("random_hashes");
    r.random_lz8 = u("random_lz8");
    r.random_lz12 = u("random_lz12");
    r.random_lz16 = u("random_lz16");
...
fn save_runtime(
    ...
    let state = json!({
        ...
        "pool": POOL,
        "worker": WORKER,
        "engine": "LIVE-04-X1331-PREDICT-COLLAPSE-VERIFY",
        ...
        "predicted_lz8": r.predicted_lz8,
        "predicted_lz12": r.predicted_lz12,
        "predicted_lz16": r.predicted_lz16,
        "random_hashes": r.random_hashes,
        "random_lz8": r.random_lz8,
        "random_lz12": r.random_lz12,
        "random_lz16": r.random_lz16,
        "gain_lz8": if r.random_lz8 > 0 { r.predicted_lz8 as f64 / r.random_lz8 as f64 } else { 0.0 },
        "gain_lz12": if r.random_lz12 > 0 { r.predicted_lz12 as f64 / r.random_lz12 as f64 } else { 0.0 },
        "gain_lz16": if r.random_lz16 > 0 { r.predicted_lz16 as f64 / r.random_lz16 as f64 } else { 0.0 },
        "submit_id": r.submit_id,
        "difficulty": difficulty,
        "job_id": job.map(|j| j.job_id.as_str()),
        "extranonce2": job.map(|j| j.extranonce2.as_str()),
        "predict_batch": job.map(|j| j.batch_counter),
...
fn header_seed(header: &[u8; 80]) -> u64 {
    let mut x = 0x1331_1331_1331_1331u64;
...
fn predict_and_collapse(net: &Net, header: &[u8; 80], batch: u64) -> (Vec<(u32, f32)>, Vec<u32>) {
    // TOP arm: score N candidates without SHA256d, then collapse to K.
    let seed = header_seed(header) ^ mix64(batch ^ 0x1331_0004_A11C_E001);
    let mut scored = Vec::with_capacity(CANDIDATE_POOL);
    for i in 0..CANDIDATE_POOL {
        let nonce = mix64(seed ^ i as u64) as u32;
        let score = net.predict(header, nonce);
        scored.push((nonce, score));
    }
    scored.sort_unstable_by(|a, b| b.1.partial_cmp(&a.1).unwrap_or(std::cmp::Ordering::Equal));
    scored.truncate(COLLAPSE_K);

    // RANDOM arm: independent deterministic nonces, same verification budget K.
    // No SHA256d is used to construct either arm.
    let control_seed = header_seed(header) ^ mix64(batch ^ 0x1331_0004_DEAD_BEEF);
    let mut random = Vec::with_capacity(COLLAPSE_K);
    for i in 0..COLLAPSE_K {
        random.push(mix64(control_seed ^ i as u64) as u32);
    }
    (scored, random)
}
...
fn predictive_batch(
    ...
    // Critical X1331 invariant: ranking happens BEFORE SHA256d.
    // TOP and RANDOM receive exactly K cryptographic verifications each.
    let (selected, random) = predict_and_collapse(observer, &job.header, job.batch_counter);
    let best_score = selected.first().map(|x| x.1).unwrap_or(0.0);
    let worst_score = selected.last().map(|x| x.1).unwrap_or(0.0);

    for (nonce, _predicted_score) in selected {
        ...
        if hash_meets_share_target(&hash, &job.target) {
            r.shares_found += 1;
            println!(
                "*** TOP POOL SHARE FOUND *** job={} ex2={} nonce={} raw_hash={} LZ={}",
                job.job_id,
                job.extranonce2,
    ...
    for nonce in random {
        ...
        if hash_meets_share_target(&hash, &job.target) {
            r.shares_found += 1;
            println!(
                "*** RANDOM POOL SHARE FOUND *** job={} ex2={} nonce={} raw_hash={} LZ={}",
                job.job_id,
                job.extranonce2,
                nonce_submit_hex(nonce),
                hex::encode(hash),
    ...
    job.batch_counter = job.batch_counter.wrapping_add(1);
    r.work_units += 1;
    if r.work_units % 10 == 0 {
        let g8 = if r.random_lz8 > 0 {
            r.predicted_lz8 as f64 / r.random_lz8 as f64
        } else {
            0.0
        };
        let g12 = if r.random_lz12 > 0 {
            r.predicted_lz12 as f64 / r.random_lz12 as f64
        } else {
            0.0
        };
        let g16 = if r.random_lz16 > 0 {
            r.predicted_lz16 as f64 / r.random_lz16 as f64
        } else {
            0.0
        };
        println!(
            "X1331 | batch {} | N={} -> TOP K={} + RANDOM K={} | score {:.6}..{:.6} | TOP LZ8/12/16={}/{}/{} | RANDOM={}/{}/{} | G={:.4}/{:.4}/{:.4}",
            job.batch_counter, CANDIDATE_POOL, COLLAPSE_K, COLLAPSE_K, best_score, worst_score,
            r.predicted_lz8, r.predicted_lz12, r.predicted_lz16,
            r.random_lz8, r.random_lz12, r.random_lz16, g8, g12, g16
        );
    }
    Ok(())
}
...
fn run_session(observer: &Net, r: &mut Runtime) -> Result<(), Box<dyn Error>> {
    ...
    loop {
            ...
            match rx.try_recv() {
                Ok(msg) => {
                    ...
                    if method == Some("mining.notify") {
                        ...
                        println!(
                            "NEW JOB {} | diff {} | clean={} | ex2={} | X1331 predictive N={} -> K={}",
                            j.job_id, d, j.clean_jobs, j.extranonce2, CANDIDATE_POOL, COLLAPSE_K
                        );
        ...
        if let Some(job) = current_job.as_mut() {
            predictive_batch(observer, &mut stream, r, &mut pending, &mut share_log, job)?;
        } else {
        ...
        if last_report.elapsed() >= Duration::from_secs(REPORT_SECS) {
            ...
            println!(
                "VERIFY | {:.0} SHA/s | total {} | TOP LZ8/12/16 {}/{}/{} | RANDOM {}/{}/{} | G8 {:.4} | shares F/S/A/R {}/{}/{}/{} | pending {}",
                hs, r.total_hashes,
                r.predicted_lz8, r.predicted_lz12, r.predicted_lz16,
                r.random_lz8, r.random_lz12, r.random_lz16,
                if r.random_lz8 > 0 { r.predicted_lz8 as f64 / r.random_lz8 as f64 } else { 0.0 },
                r.shares_found, r.shares_submitted, r.shares_accepted, r.shares_rejected,
                pending.len()
            );

            save_runtime(r, "PREDICT-COLLAPSE-VERIFY", difficulty, job_ref, hs)?;
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    println!();
    println!("X1331 LIVE-04 — PREDICT / COLLAPSE / VERIFY + RANDOM CONTROL");
    println!("====================================");
    println!("Observer        : LIVE-02 persistent model, inference only (frozen during run)");
    println!("Pipeline        : Generate N -> Predict -> TOP K + RANDOM K -> SHA256d verify");
    println!("Candidate pool  : {}", CANDIDATE_POOL);
    println!("Collapse K      : {}", COLLAPSE_K);
    println!("Pool            : {}", POOL);
