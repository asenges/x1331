...
fn reverse32(mut v: Vec<u8>) -> Result<[u8; 32], Box<dyn Error>> {
    if v.len() != 32 {
...
fn main() -> Result<(), Box<dyn Error>> {
    println!("X1331 Binance Stratum Adapter — DRY RUN");
    println!("======================================");
    println!("Pool   : {POOL}");
    ...
    loop {
        ...
        let prevhash = reverse32(hex::decode(prevhash_hex)?)?;
        ...
        // sha256d() gives digest bytes; Bitcoin header serialization
        // uses the internal-order merkle root.
        let mut merkle_header = merkle;
        merkle_header.reverse();
        header[36..68].copy_from_slice(&merkle_header);
